...
import { formatDuration, relativeTime } from "@/shared/utils/date";
...
import { type CallRecord, useCallsStore } from "./store";
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        {isLoading && calls.length === 0 ? (
          ...
          <div style={{ padding: "0 var(--space-sm)" }}>
            {calls.map((call, i) => (
              <CallItem key={call.id} call={call} index={i} />
            ))}
          </div>
        )}
      </PullToRefresh>
...
const CallItem = memo(function CallItem({ call, index }: { call: CallRecord; index: number }) {
  ...
  const icon = directionIcon[call.direction] ?? { symbol: "\u2199", color: "var(--color-success)" };
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
          ...
          <span
            style={{
              ...
              color:
                call.direction === "missed" ? "var(--color-danger)" : "var(--color-text-primary)",
              overflow: "hidden",
