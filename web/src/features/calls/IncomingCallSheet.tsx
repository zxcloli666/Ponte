        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-md)" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: "var(--font-size-body)", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {displayName}
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                <span style={{ fontSize: "var(--font-size-caption)", color: "var(--color-text-tertiary)" }}>Входящий вызов</span>
                <LineBadge simId={activeCall.simId} extraNumberId={activeCall.extraNumberId} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <motion.button type="button" onClick={handleDecline} whileTap={{ scale: 0.9 }}
                style={{ width: 40, height: 40, borderRadius: "50%", background: "var(--color-danger)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", border: "none" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="#fff" style={{ transform: "rotate(135deg)" }}>
                  <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" />
                </svg>
              </motion.button>
              <motion.button type="button" onClick={handleAccept} whileTap={{ scale: 0.9 }}
                style={{ width: 40, height: 40, borderRadius: "50%", background: "var(--color-success)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", border: "none", boxShadow: "0 0 16px rgba(52,199,89,0.4)" }}>
