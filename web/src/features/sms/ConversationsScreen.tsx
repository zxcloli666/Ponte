...
import { getConversations, deleteConversation as deleteConvApi } from "./api";
import { useSmsStore, type Conversation } from "./store";
...
export default function ConversationsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try { const d = await getConversations(); if (!c) setConversations(d.items); }
  ...
  const handleRefresh = useCallback(async () => {
    try { const d = await getConversations(); setConversations(d.items); } catch {}
  }, [setConversations]);

  const filtered = search
    ? conversations.filter(c => (c.contactName?.toLowerCase().includes(search.toLowerCase()) ?? false) || c.address.includes(search))
    : conversations;
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        {isLoading && conversations.length === 0 ? (
          ...
          <EmptyState
            icon={<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg>}
          ...
          <div style={{ padding: "0 var(--space-sm)" }}>
            <AnimatePresence>
              {filtered.map((conv, i) => <ConvItem key={conv.address} conv={conv} index={i} />)}
            </AnimatePresence>
          </div>
        )}
      </PullToRefresh>
...
const ConvItem = memo(function ConvItem({ conv, index }: { conv: Conversation; index: number }) {
  ...
  const click = useCallback(() => navigate(`/sms/${encodeURIComponent(conv.address)}`), [navigate, conv.address]);
  ...
  return (
    <motion.div
      ...
      onDragEnd={(_e, info) => { if (info.offset.x < -100) handleDelete(); }}
      style={{
        width: "100%", display: "flex", alignItems: "center", gap: "var(--space-md)",
        padding: "var(--space-md)", textAlign: "left", cursor: "grab",
        borderRadius: "var(--radius-lg)", marginBottom: 2, position: "relative",
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
          <span style={{ fontSize: "var(--font-size-body)", fontWeight: conv.unreadCount > 0 ? 600 : 400, color: "var(--color-text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
            {name}
          </span>
          <span style={{ fontSize: "var(--font-size-caption)", color: "var(--color-text-tertiary)", marginLeft: 8, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <span style={{
              ...
              color: conv.unreadCount > 0 ? "var(--color-text-primary)" : "var(--color-text-secondary)",
              fontWeight: conv.unreadCount > 0 ? 500 : 400,
              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block",
            }}>
              {conv.lastMessage.direction === "outgoing" && <span style={{ color: "var(--color-text-tertiary)" }}>Вы: </span>}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
              ...
              {isOtp && (
                <span style={{
                  fontSize: "var(--font-size-caption2)", fontWeight: 600,
                  background: "var(--gradient-accent-subtle)", color: "var(--color-accent)",
