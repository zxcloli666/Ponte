...
import { type ContactPhone, useContactsStore } from "./store";
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        >
          ...
          <h1
            style={{ fontSize: 28, fontWeight: 600, textAlign: "center", letterSpacing: "-0.01em" }}
          >
            {contact.name}
          </h1>
        </motion.div>
          ...
          <Glass variant="card" padding="none" style={{ overflow: "hidden" }}>
            {contact.phones.map((phone, i) => (
              <PhoneRow
                key={`${phone.number}-${i}`}
                phone={phone}
                isLast={i === contact.phones.length - 1}
              />
            ))}
          </Glass>
        </div>
...
const PhoneRow = memo(function PhoneRow({
  phone,
  isLast,
}: { phone: ContactPhone; isLast: boolean }) {
  const navigate = useNavigate();
  const typeLabel = PHONE_TYPE_LABELS[phone.type.toLowerCase()] ?? (phone.label || phone.type);

  const handleCall = useCallback(
    () => navigate(`/calls/dialer?number=${encodeURIComponent(phone.number)}`),
    [navigate, phone.number],
  );
  const handleMessage = useCallback(
    () => navigate(`/sms/${encodeURIComponent(phone.number)}`),
    [navigate, phone.number],
  );
