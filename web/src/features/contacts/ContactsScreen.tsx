...
export default function ContactsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try { const d = await getContacts(1, 100); if (!c) setContacts(d.items); }
  ...
  const handleRefresh = useCallback(async () => {
    try { const d = await getContacts(1, 100); setContacts(d.items); } catch {}
  }, [setContacts]);
      ...
      <div style={{ flex: 1, position: "relative" }}>
        <PullToRefresh onRefresh={handleRefresh}>
          {isLoading && totalCount === 0 ? (
            ...
            <EmptyState
              icon={<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>}
              title={searchQuery ? "Ничего не найдено" : "Нет контактов"}
              description={searchQuery ? "Попробуйте другой запрос" : "Контакты с Android появятся здесь"}
            />
          ) : (
              ...
              {sections.map((section) => (
                <div key={section.letter} id={`section-${section.letter}`}>
                  <div style={{
                    ...
                    fontSize: "var(--font-size-footnote)", fontWeight: 700,
                    color: "var(--color-text-secondary)",
                    background: "var(--glass-bg-solid)", backdropFilter: "blur(var(--glass-blur))",
                  }}>
                    {section.letter}
                  </div>
...
const ContactItem = memo(function ContactItem({ contact, index }: { contact: Contact; index: number }) {
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        padding: "var(--space-sm) var(--space-md)", textAlign: "left", cursor: "pointer",
        background: "var(--glass-bg)", border: "1px solid var(--glass-border)",
        borderRadius: "var(--radius-lg)", marginBottom: 2,
        transition: "background var(--duration-fast)",
      }}
    >
      <Avatar name={contact.name} photoUrl={contact.photoUrl} size={40} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <span style={{ fontSize: "var(--font-size-body)", fontWeight: 400, color: "var(--color-text-primary)", display: "block" }}>
          {contact.name}
        </span>
        {primaryPhone && (
...
const AlphabetScrubber = memo(function AlphabetScrubber({ alphabet, onSelect }: { alphabet: string[]; onSelect: (l: string) => void }) {
  ...
  return (
    ...
    >
      {alphabet.map((letter) => (
        <button
          key={letter} type="button" data-letter={letter}
